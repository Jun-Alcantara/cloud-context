...
3. Run `kanban_manage` with `action: "list_boards"` to get all boards.
4. Present the boards as a numbered list. Ask the user which board they want to view, or if they want to create a new one.
5. If they pick a board, run `kanban_manage` with `action: "get_board"` and the chosen `boardId`. Present the board as a structured table with columns as headings and tasks listed under each.
6. If they want to create a board, ask for the name, then run `kanban_manage` with `action: "create_board"`.
